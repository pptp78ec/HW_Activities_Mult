...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ListView>(R.id.carlist).adapter = ModelListAdapter(this, R.layout.listitem, carObjects)
    }
        ...
        if(item.itemId == R.id.menuitem_filter){
            ...
            val result  = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
                if(it.resultCode == Activity.RESULT_OK){
                    carObjects =  (it.data?.extras?.get("retObject") as Array<ModelAuto>).toMutableList()
                    recreate()
            }
            return true
        }
